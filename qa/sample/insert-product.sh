...
# ----------------------------------------------------
echo ""
echo "1단계: generate-product.js를 사용하여 SQL 파일 생성"
# Node.js 스크립트 실행
...
if command -v mysql &> /dev/null; then
    ...
    mysql -h "$DB_HOST" -P "$DB_PORT" -u "$DB_USER" -p"$DB_PASSWORD" --default-character-set=utf8mb4 "$DB_NAME" < results/products-data.sql
    echo "데이터 삽입 완료!"
    ...
    if [ -n "$MYSQL_CONTAINER" ]; then
        ...
        docker exec -i "$MYSQL_CONTAINER" mysql -u "$DB_USER" -p"$DB_PASSWORD" --default-character-set=utf8mb4 "$DB_NAME" < results/products-data.sql
        echo "데이터 삽입 완료!"
    else
