...
public interface PgSimulatorClient {
    ...
    PgApiResponse<PgSimulatorResponse.RequestTransaction> request(
            @RequestHeader(HEADER_USER_ID) String userId,
            PgSimulatorRequest.RequestTransaction requestTransaction
    );

    @GetMapping("/api/v1/payments/{transactionKey}")
    PgApiResponse<PgSimulatorResponse.TransactionDetail> getTransaction(@RequestHeader(HEADER_USER_ID) String userId, @PathVariable String transactionKey);
}
